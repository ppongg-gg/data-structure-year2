...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		if(((TreeIterator) subtree.findMax(subtree.root)).currentNode.data >= ((TreeIterator) findMin(root)).currentNode.data) return;
		TreeIterator p = (TreeIterator) findMin(root);
		subtree.root.parent = p.currentNode;
		p.currentNode.left = subtree.root;
		size += subtree.size;
